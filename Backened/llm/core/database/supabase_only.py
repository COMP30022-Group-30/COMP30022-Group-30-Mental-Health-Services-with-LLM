...
class SupabaseOnlyConnection:
    ...
    def search_services_by_text(
        ...
        try:
            # Accept either a simple query string or a JSON payload with filters.
            payload: Any = search_term
            ...
            if isinstance(payload, dict):
                ...
                if raw_terms is None:
                    raw_terms = payload.get("search_term", [])
                ...
                location_filter = payload.get("location_filter") or None
                service_filter = payload.get("service_filter") or None
                cost_filter = payload.get("cost_filter") or None
            elif isinstance(payload, (list, tuple)):
                search_terms = [str(term).strip() for term in payload if str(term).strip()]
            else:
                term = str(payload).strip()
                if term:
                    search_terms = [term]

            logger.info(
                "Performing service text search",
